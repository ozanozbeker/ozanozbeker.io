...
When a dataset contains a categorical variable and one or more continuous variables, we might be interested in the distribution of the continuous variable(s) relative to the categorical variable.
...
Because there a few unique number of values for both `drv` and `hwy`, there is a lot of overplotting (where one or more points are plotted in the same location). There are a few useful techniques to help with this issue:

-   *Jitter Plots*, `geom_jitter()`, add a little random noise to the data which can help avoid overplotting.
...
```{r}
#| label: 2.6.2 Example different plots
#| layout-nrow: 1
...
Each method has its strength and weaknesses:

-   *Jitter Plots* show every point but only work with relatively small datasets.
-   *Boxplots* summarize the whole distribution with 5 statistics.
-   *Violin Plots* give the richest display, but the density estimate can be hard to interpret.

#### Bonus

It's not mentioned here in the book, but you can also adjust the transparency, `alpha`, of the geom to also help with this issue. This is especially useful when comparing two different continuous variables where the geoms above are not able to be plotted.

```{r}
#| label: 2.6.2 Example different alphas
#| layout-nrow: 1
...
It is highly recommended to experiment with the `binwidth` as the default value is 30 and it is unlikely that 30 is the best choice for your dataset.
...
For example, you might have three drugs with the their *average* effect:

```{r}
#| label: 2.6.4 Example drug data

drugs = tibble(drug = c("a", "b", "c"), effect = c(4.2, 9.7, 6.1))
```

To display this type of data, you have to tell `geom_bar()` to not run the default stat which bins and counts data. In this case, it's better to use `geom_point()` because it takes up less space than bars, and don't require that the axis includes 0.
...
To compare the relationship, we would like to draw both time series on the same plot. We could draw a scatterplot of unemployment rate vs length of time unemployed, but then we lose the dimension of time. The solution is to join points adjacent in time with line segments, forming a *path* plot. We will add some aesthetics to help see the direction of time.
